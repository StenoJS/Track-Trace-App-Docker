...
// Publieke track&trace-URL's, alleen voor koeriers waar een kale code
// (zonder extra gegevens zoals postcode) genoeg is om iets bruikbaars te
// tonen. DHL en DPD zijn overgenomen uit echte notificatiemails (dus
// bevestigd werkend), UPS/PPL uit hun eigen documentatie/site.
// PostNL laten we bewust weg: hun track&trace vereist ook de postcode van
// de ontvanger, die we nergens opslaan -- een link zonder postcode komt op
// een leeg formulier uit, dus dan is de kale code nuttiger.
...
function buildTrackingUrl(courier, code) {
...
export function formatTelegramMessage(parsed) {
  ...
  if (parsed.trackingCode) {
    ...
    // invoeren elders.
    const url = buildTrackingUrl(parsed.courier, parsed.trackingCode);
    if (url) {
