...
import { STATUS_KEYWORDS_RE, findKnownCodeShape } from "./parse.js";

// Bekende koeriersdomeinen (tier 1, zie parse.js) -- deze mail komt sowieso
// door de pre-filter, ongeacht of het onderwerp een statuswoord bevat.
...
// Goedkope pre-filter op envelope-niveau (geen body nodig): laat door als de
// afzender een bekende koerier is, het onderwerp al naar een verzendupdate
// ruikt, óf het onderwerp al een bekende trackingcode-vorm bevat (DHL zet
// die standaard tussen haakjes in het onderwerp -- blijft ook staan bij een
// doorgestuurde mail, ook als de rest van het onderwerp geen statuswoord
// bevat dat we kennen, bv. "We staan vandaag voor de deur..."). Dit
// voorkomt dat we voor élke mail in de inbox de body gaan downloaden.
function isLikelyShippingMail(fromAddr, subject) {
  return isCourierSender(fromAddr) || STATUS_KEYWORDS_RE.test(subject || "") || !!findKnownCodeShape(subject || "");
}
...
/**
 * Haalt nieuwe koeriers-/verzendmails op uit INBOX sinds sinceDate.
...
export async function fetchNewCourierMails({ host, port, secure, user, pass, sinceDate, excludeMessageIds }) {
  ...
  try {
    ...
    try {
      ...
      // Fase 1: goedkope envelope-only pass, filteren op afzender/onderwerp + dedupe.
      const matches = [];
      for await (const msg of client.fetch(searchCriteria, { envelope: true, uid: true })) {
        const fromAddr = msg.envelope?.from?.[0]?.address;
        const fromName = msg.envelope?.from?.[0]?.name;
        const subject = msg.envelope?.subject || "";
        if (!isLikelyShippingMail(fromAddr, subject)) continue;
